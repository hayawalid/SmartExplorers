...
class MatchResult:
    ...
    def to_dict(self):
        ...
        return {
            ...
            "match_score": float(self.match_score),  # Convert numpy float to Python float
            "match_quality": self.match_quality,
            "match_reasons": self.match_reasons,
            "common_interests": self.common_interests,
            "common_languages": self.common_languages,
            "common_dates": self.common_dates,
            "budget_compatibility": float(self.budget_compatibility),
            "safety_score": float(self.safety_score),
            "demographics_bonus": float(self.demographics_bonus),
            "interest_similarity": float(self.interest_similarity),
            "cluster_id": int(self.cluster_id),  # Convert numpy int to Python int
...
class SmartMatchingEngine:
    ...
    def _calculate_interest_similarity(self, user1: Dict, user2: Dict) -> float:
        ...
        """
        # Get profile data (guard against None)
        profile1 = (user1.get("profile") or {}) if user1.get("account_type") == "traveler" else (user1.get("provider_profile") or {})
        profile2 = (user2.get("profile") or {}) if user2.get("account_type") == "traveler" else (user2.get("provider_profile") or {})
        
        # Get interests
        interests1 = set(profile1.get("travel_interests") or profile1.get("setup_interests") or profile1.get("services_offered") or [])
        interests2 = set(profile2.get("travel_interests") or profile2.get("setup_interests") or profile2.get("services_offered") or [])
        
        # Get bios
        bio1 = (user1.get("bio") or profile1.get("bio") or "").lower()
        bio2 = (user2.get("bio") or profile2.get("bio") or "").lower()
    ...
    def _extract_demographic_features(self, user: Dict[str, Any]) -> np.ndarray:
        ...
        """
        profile = (user.get("profile") or {}) if user.get("account_type") == "traveler" else (user.get("provider_profile") or {})
        
        # Language encoding (11 features)
        language_vector = np.zeros(len(self.common_languages))
        languages = profile.get("languages_spoken") or profile.get("languages") or []
...
Be honest and critical. A poor match is better than a forced match."""
    ...
    def find_matches(
        ...
        target_profile = (target_user.get("profile") or {}) if target_type == "traveler" else (target_user.get("provider_profile") or {})
        ...
        for candidate in candidate_users:
            ...
            if candidate_type == "service_provider":
                candidate_profile = candidate.get("provider_profile", {}) or {}
            
            # Check languages – default to ["English"] when not set
            target_languages = target_profile.get("languages_spoken") or target_profile.get("languages") or ["English"]
            candidate_languages = (
                candidate_profile.get("languages") if candidate_type == "service_provider"
                else candidate_profile.get("languages_spoken")
            ) or ["English"]
            ...
            if target_type == "traveler":
                target_interests = (target_profile.get("travel_interests") or []) + (target_profile.get("setup_interests") or [])
            else:
                target_interests = target_profile.get("services_offered") or []
                
            if candidate_type == "traveler":
                candidate_interests = (candidate_profile.get("travel_interests") or []) + (candidate_profile.get("setup_interests") or [])
            else:
                candidate_interests = candidate_profile.get("services_offered") or []
    ...
    def get_match_statistics(self, matches: List[MatchResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality_distribution": {str(k): int(v) for k, v in quality_counts.items()},
            "perfect_matches": int(quality_counts.get("PERFECT", 0)),
            "great_matches": int(quality_counts.get("GREAT", 0)),
            "good_matches": int(quality_counts.get("GOOD", 0)),
            "matches_above_90": int(sum(1 for s in scores if s >= 0.9)),
            "matches_above_80": int(sum(1 for s in scores if s >= 0.8)),
            "matches_above_50": int(sum(1 for s in scores if s >= 0.5)),
            "top_interests": [[str(k), int(v)] for k, v in Counter(all_interests).most_common(10)],
