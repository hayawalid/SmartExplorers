...
# Thread pool for running synchronous engine methods without blocking the event loop
...
def match_result_to_response(match: MatchResult, user_dict: Dict[str, Any]) -> MatchResponseItem:
    ...
    return MatchResponseItem(
        ...
        common_languages=list(match.common_languages),
        common_dates=list(match.common_dates),
        budget_compatibility=float(match.budget_compatibility),
        safety_score=float(match.safety_score),
        demographics_bonus=float(match.demographics_bonus),
...
async def train_matching_model(request: TrainClusterRequest):
    ...
    try:
        ...
        cluster_labels = await loop.run_in_executor(
            _executor,
            lambda: matching_engine.train_clusters(users, n_clusters=request.n_clusters),
        )
        _model_trained = True
...
async def find_matches(request: MatchRequest):
    """Find best matches for a given user (uses engine's own DB connection, same as test_matching_system.py)."""
    global _model_trained

    try:
        loop = asyncio.get_event_loop()

        # Fetch all users via the engine's synchronous MongoDB connection
        all_users = await loop.run_in_executor(_executor, matching_engine.fetch_all_users)
        print(f"[find-matches] Fetched {len(all_users)} users, looking for '{request.user_email}'")

        # Find the target user by email
        target_user = next((u for u in all_users if u.get("email") == request.user_email), None)
        if not target_user:
            # Fallback: try from the async app DB
            print(f"[find-matches] Not found via engine, trying async DB fallback...")
            db = get_database()
            all_users = await fetch_all_users(db)
            print(f"[find-matches] Async fallback fetched {len(all_users)} users")
            target_user = next((u for u in all_users if u.get("email") == request.user_email), None)
            if not target_user:
                all_emails = [u.get("email") for u in all_users]
                print(f"[find-matches] Available emails: {all_emails}")
                raise HTTPException(status_code=404, detail=f"User {request.user_email} not found")

        # Auto-train if model is not yet trained
        if not _model_trained:
            n = min(5, len(all_users))
            if len(all_users) < 2:
                raise HTTPException(status_code=400, detail="Not enough users to train model.")
            await loop.run_in_executor(
                _executor,
                lambda: matching_engine.train_clusters(all_users, n_clusters=n),
            )
            _model_trained = True
        ...
        matches = await loop.run_in_executor(
            _executor,
            lambda: matching_engine.find_matches(
...
async def get_model_status():
    ...
    return {
        ...
        "groq_available": matching_engine.groq_api_key is not None,
    }
...
async def initialize_matching_system(db=None, n_clusters: int = 5):
    """Initialize the matching system on startup using the engine's own DB connection."""
    global _model_trained

    try:
        users = matching_engine.fetch_all_users()
